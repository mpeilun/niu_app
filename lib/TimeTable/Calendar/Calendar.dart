...
//List< Map<Class,Calendar> >
  List<Map<String, String>> saveList = [
    {
      Class("電子電路", "朱志明", "教416", 1, 2, 4).save():
          Calendar(1, null, null).save()
    }
  ];
  void push(Class cl, Calendar ca, int? week) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    if (week == null) week = prefs.getInt("SelectWeek")!;
    if (week == -1) week = 0;
  ...
  void del(Class cl, int? week) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    if (week == null) week = prefs.getInt("SelectWeek")!;
    if (week == -1) week = 0;
    List<String> saveList = await readMen();
  ...
  Map<String, String> stringToMap(String str) {
    if (str.isNotEmpty) return Map<String, String>.from(json.decode(str));
    return {};
  }

  String mapToString(Map<String, String> map) {
    if (map.isNotEmpty) return json.encode(map);
    return "";
  }
  ...
  Class strToClass(String str) {
    ...
    return Class(
        className, teacherName, classroom, weekDay, startTime, endTime);
  }
